...
export default function useAppState() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [u, p, c, v, t, b, sUrl, dp, cr, al, tid, ak, cc, bgi, cs, lu, invS, sess, storedAliases, pay, so, po, exp, inc] = await Promise.all([
          ...
          lsGet(SK.payments, []),
          lsGet(SK.salesOrders, []),
          lsGet(SK.purchaseOrders, []),
          lsGet(SK.expenses, []),
          lsGet(SK.incomes, [])
        ]);
  ...
  const saveSalesOrders = async o => { setSalesOrders(o); await lsSet(SK.salesOrders, o); };
  const savePurchaseOrders = async o => { setPurchaseOrders(o); await lsSet(SK.purchaseOrders, o); };
  const saveExpenses = async e => { setExpenses(e); await lsSet(SK.expenses, e); };
  const saveIncomes = async i => { setIncomes(i); await lsSet(SK.incomes, i); };
  ...
  return {
    ...
    ctx: {
      ...
      salesOrders, saveSalesOrders, purchaseOrders, savePurchaseOrders,
      expenses, saveExpenses, incomes, saveIncomes,
