...
export default function BillForm({ type, bills, onSave, products, vendors, getStock, existingBill, invoiceSettings, seriesOverride }) {
  const T = useT();
  ...
  const billNo = useMemo(() => {
    ...
    if (type === "sale") {
      const prefix = seriesOverride?.prefix || invoiceSettings?.saleSeries || "SALE-";
      const startNum = Number(seriesOverride?.start ?? invoiceSettings?.saleSeriesStart ?? 1);
      // Filter to bills matching this prefix (distinguish proforma vs tax invoice counters)
      const sameSeriesBills = (bills||[]).filter(b =>
        b.type === "sale" &&
        (seriesOverride?.isProforma ? b.isProforma : !b.isProforma) &&
        (b.billNo || "").startsWith(prefix)
      );
      let maxNum = startNum - 1;
      sameSeriesBills.forEach(b => {
  ...
  }, [bills, type, isEdit, existingBill, invoiceSettings, purchaseInvoiceNo, seriesOverride]);
    ...
    <form id={type + "-form"} onSubmit={e => { e.preventDefault(); handleSave(); }} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div className="glass" style={{ borderRadius: T.radius, overflow: "visible", padding: 0 }}>
        ...
        <div style={{ overflow: "visible" }}>
          ...
          <div className="hide-mob" style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) 70px 110px 90px 32px", gap: 10, padding: "10px 14px", background: T.isDark ? "rgba(255,255,255,0.02)" : "rgba(0,0,0,0.02)", borderBottom: `1px solid ${T.borderSubtle}` }}>
            {["Product", "Qty", type === "sale" ? "MRP (incl GST)" : "Cost (ex-GST)", "Line Total", ""].map((h, i) => (
              <div key={i} style={{ fontSize:10, fontWeight: 800, color: T.textMuted, letterSpacing: "0.05em", textTransform: "uppercase", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{h}</div>
            ))}
          </div>
          ...
          {items.map((item, i) => {
            ...
            return (
              <div key={item.id} className="liquid-trans" style={{ borderBottom: `1px solid ${T.borderSubtle}`, padding: "12px 14px", background: T.surfaceGlass, overflow: "visible" }}>
                
                {/* Desktop View */}
                <div className="hide-mob" style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) 70px 110px 90px 32px", gap: 10, alignItems: "start", overflow: "visible" }}>
