...
export default function BillForm({ type, bills, onSave, products, vendors, getStock, existingBill, invoiceSettings }) {
  ...
  return (
    <form id={type + "-form"} onSubmit={e => { e.preventDefault(); handleSave(); }} style={{ display: "flex", flexDirection: "column", gap: 20 }}>

      <div className="glass" style={{ padding: "12px 16px", borderRadius: T.radius, background: T.accentBg, display: "flex", alignItems: "center", gap: 10, border: `1px solid ${T.accent}30` }}>
        <div style={{ width: 28, height: 28, borderRadius: "50%", background: T.accent, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 2px 8px ${T.accent}50` }}>
      ...
      {type === "purchase" && (
        <div className="glass" style={{ padding: "16px 20px", borderRadius: T.radius }}>
      ...
      <div className="glass fgrid" style={{ padding: "20px", borderRadius: T.radius }}>
        ...
        {selectedVendor && (
          <div className="s2 spring-down" style={{ padding: "12px 16px", borderRadius: T.radius, background: T.isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)", border: `1px solid ${T.borderSubtle}`, fontSize: 12, color: T.textSub, lineHeight: 1.5 }}>
      ...
      <div className="glass" style={{ padding: "16px 20px", borderRadius: T.radius, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, fontWeight: 800, color: T.textSub, letterSpacing: "0.05em" }}>GST TYPE</span>
        {gstAutoSet
          ? <div className="spring-in" style={{ display:"flex", alignItems:"center", gap:12 }}>
              <span style={{ padding:"6px 14px", borderRadius:T.radiusFull, background:gstType==="igst"?T.blueBg:T.greenBg, color:gstType==="igst"?T.blue:T.green, fontSize:12, fontWeight:700, boxShadow: `0 2px 8px ${gstType==="igst"?T.blue:T.green}30` }}>
              ...
              <button type="button" className="liquid-trans" onClick={() => setGstAutoSet(false)} style={{ fontSize:12, color:T.accent, background:"none", border:"none", cursor:"pointer", fontWeight: 700 }}>Change</button>
            </div>
          : <div style={{ display:"flex", gap:4, background:T.isDark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.04)", borderRadius:T.radius, padding:4 }}>
              {[{k:"cgst_sgst",l:"CGST + SGST",sub:"Intra-state"},{k:"igst",l:"IGST",sub:"Inter-state"}].map(g=>(
                <button type="button" className="liquid-trans" key={g.k} onClick={()=>setGstType(g.k)} style={{ padding:"6px 16px", borderRadius:T.radius, border:"none", cursor:"pointer", background:gstType===g.k?T.accent:"transparent", color:gstType===g.k?"#fff":T.textSub, fontSize:12, fontWeight:700, boxShadow: gstType===g.k ? `0 2px 8px ${T.accent}50` : "none" }}>
                  {g.l} <span style={{fontSize:11,opacity:.8, fontWeight: 500, marginLeft: 4}}>({g.sub})</span>
                </button>
              ))}
            </div>
        }
      ...
      {type === "sale" && (
        <div className="glass" style={{ padding: "16px 20px", borderRadius: T.radius }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: shipToSameAsBill ? 0 : 16 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: T.isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Truck size={14} color={T.textSub} />
            </div>
            <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>Shipping Address</span>
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <label htmlFor="sameAsBill" style={{ fontSize: 12, color: T.textSub, cursor: "pointer", fontWeight: 600 }}>Same as Billing</label>
            </div>
          </div>
          {!shipToSameAsBill && (
            <div className="spring-down fgrid" style={{ paddingTop: 16, borderTop: `1px solid ${T.borderSubtle}` }}>
              ...
              <div key={item.id} className="liquid-trans" style={{ borderBottom: `1px solid ${T.borderSubtle}`, padding: "16px 20px", background: T.surfaceGlass, overflow: "visible" }}>
                ...
                <div className="hide-mob" style={{ display: "grid", gridTemplateColumns: "1fr 80px 120px 100px 32px", gap: 12, alignItems: "start", overflow: "visible" }}>
                  ...
                  <div>
                    ...
                    {rate > 0 && lineBase > 0 && <div className="spring-in" style={{ fontSize:11, marginTop: 6, color: T.textMuted, fontWeight: 500 }}>{type === "purchase" ? `+GST: ${fmtCur(lineGst)}` : `GST: ${fmtCur(lineGst)}`}</div>}
                  </div>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 800, color: T.text, padding: "10px 0" }}>{fmtCur(lineTotal)}</div>
                    {discAmt > 0 && lineBase > 0 && type === "sale" && <div className="spring-in" style={{ fontSize:11, color: T.textMuted, fontWeight: 500 }}>was {fmtCur(lineBase)}</div>}
                  </div>
                  <button type="button" onClick={() => remItem(item.id)} className="btn-danger liquid-trans" style={{ width: 32, height: 36, padding: 0, opacity: items.length <= 1 ? .3 : 1 }} disabled={items.length <= 1}>
                    <X size={14} />
                  </button>
                </div>
                ...
                <div className="bill-item-sub">
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 800, color: T.text, padding: "8px 0" }}>{fmtCur(lineTotal)}</div>
                    </div>
                  </div>
      ...
      <div className="fgrid">
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ display: "flex", gap: 4, background: T.isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)", borderRadius: T.radius, padding: 4, flexShrink: 0 }}>
                    {[{ k: "percent", l: "%" }, { k: "amount", l: "₹" }].map(d => (
                      <button type="button" key={d.k} className="liquid-trans" onClick={() => setDiscType(d.k)} style={{ padding: "6px 14px", borderRadius: T.radius, border: "none", cursor: "pointer", background: discType === d.k ? T.accent : "transparent", color: discType === d.k ? "#fff" : T.textSub, fontSize: 13, fontWeight: 700, boxShadow: discType === d.k ? `0 2px 8px ${T.accent}40` : "none" }}>{d.l}</button>
                    ))}
                  </div>
        ...
        <div className="glass" style={{ padding: "20px", borderRadius: T.radius, display: "flex", flexDirection: "column", gap: 8, justifyContent: "center" }}>
          ...
          {type === "sale" && discAmt > 0 && (
            <div className="spring-down" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: T.red }}>
          ...
          {type === "purchase" && totalGst > 0 && (
            <div className="spring-down" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: T.amber }}>
              <span style={{ fontWeight: 600 }}>Total GST</span>
              <span style={{ fontWeight: 700 }}>+{fmtCur(totalGst)}</span>
            </div>
          )}
          {type === "sale" && totalGst > 0 && (
            <div className="spring-in" style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textMuted }}>
              <span style={{ fontWeight: 500 }}>GST incl. in total</span>
              <span style={{ fontWeight: 600 }}>{fmtCur(totalGst)}</span>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 18, fontWeight: 800, color: T.text, borderTop: `1px solid ${T.borderSubtle}`, paddingTop: 12, marginTop: 4 }}>
            <span>Grand Total</span>
            <span style={{ color: T.accent }}>{fmtCur(total)}</span>
          </div>
          {type === "sale" && totalGst > 0 && (
            <div className="spring-in" style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: T.textMuted, marginTop: 4 }}>
      ...
      {type === "sale" && (
        <div className="glass" style={{ borderRadius: T.radius, overflow: "hidden" }}>
          <button type="button" onClick={() => setEwayBill(p => !p)} className="liquid-trans" style={{ width: "100%", padding: "16px 20px", display: "flex", alignItems: "center", gap: 10, background: "transparent", border: "none", cursor: "pointer", color: T.text }}>
            <input type="checkbox" className="cb" checked={ewayBill} readOnly style={{ pointerEvents: "none" }} />
            <span style={{ fontSize: 14, fontWeight: 700 }}>E-Way Bill Required</span>
            {ewayBill ? <ChevronUp size={16} color={T.textMuted} style={{ marginLeft: "auto" }} /> : <ChevronDown size={16} color={T.textMuted} style={{ marginLeft: "auto" }} />}
          </button>
          {ewayBill && (
            <div className="spring-down fgrid" style={{ padding: "0 20px 20px" }}>
      ...
      <div className="glass" style={{ padding: "20px", borderRadius: T.radius }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, fontWeight: 800, color: T.textSub, letterSpacing: "0.05em" }}>PAYMENT MODE</span>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["Cash", "NEFT / RTGS", "UPI", "Cheque", "Credit", "On Account"].map(pm => (
              <button type="button" key={pm} className="liquid-trans" onClick={() => setPaymentMode(paymentMode === pm ? "" : pm)} style={{ padding: "6px 16px", borderRadius: T.radiusFull, fontSize: 12, fontWeight: 700, border: `1px solid ${paymentMode === pm ? T.accent : T.borderSubtle}`, cursor: "pointer", background: paymentMode === pm ? T.accent : "transparent", color: paymentMode === pm ? "#fff" : T.textSub, boxShadow: paymentMode === pm ? `0 2px 8px ${T.accent}40` : "none" }}>{pm}</button>
            ))}
          </div>
        </div>
      </div>
