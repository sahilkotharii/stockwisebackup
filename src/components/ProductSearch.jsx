...
export default function ProductSearch({ value, onChange, products, placeholder, getStock }) {
  ...
  useEffect(() => {
    ...
    // Use requestAnimationFrame-throttled scroll handler
    let ticking = false;
    const onScroll = () => {
      if (!ticking) {
        ticking = true;
        ...
        <div ref={dropRef} className="spring-in ps-dropdown" style={{
          ...
          {filtered.length === 0
            ...
            : filtered.map(p => (
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 700, color: p.id === value ? T.accent : T.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{p.name}</div>
