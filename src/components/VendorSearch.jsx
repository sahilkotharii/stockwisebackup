...
export default function VendorSearch({ value, onChange, vendors = [], placeholder = "Search vendor/customer…" }) {
  ...
  const calcPos = useCallback(() => {
    ...
    const dropW = Math.min(Math.max(r.width, 300), vw - 24);
    let left = r.left;
    if (left + dropW > vw - 12) left = vw - 12 - dropW;
    if (left < 12) left = 12;
    const spaceBelow = vh - r.bottom;
    const maxH = 300;
    let top = r.bottom + 4;
    if (spaceBelow < maxH && r.top > spaceBelow) {
      top = r.top - Math.min(maxH, r.top - 12) - 4;
    }
  ...
  useEffect(() => {
    ...
    let ticking = false;
    const onScroll = () => {
      if (!ticking) { ticking = true; requestAnimationFrame(() => { calcPos(); ticking = false; }); }
    };
  ...
  const openDropdown = () => {
    if (!isOpen) { setSearch(""); calcPos(); setIsOpen(true); }
  };
  ...
  const clear = () => { onChange(""); setSearch(""); setIsOpen(false); };
    ...
    <div ref={wrapRef} style={{ position: "relative" }}>
      <div style={{ position: "relative" }}>
        ...
        <input
          ...
          onChange={e => { if (!isOpen) openDropdown(); setSearch(e.target.value); }}
        />
        {value && !isOpen && (
      ...
      {isOpen && createPortal(
        <div ref={dropRef} className="spring-in ps-dropdown" style={{
          ...
          {filtered.map(v => (
            <div key={v.id}
              ...
              onTouchEnd={e => { e.preventDefault(); pick(v.id); }}
              style={{
