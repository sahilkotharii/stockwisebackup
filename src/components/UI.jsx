...
/* ── Toast — sits above mobile bottom nav ────────────────────── */
export function Toast({ msg, type }) {
  const T = useT();
  const c = type === "error" ? T.red : type === "success" ? T.green : T.amber;
  return <div className="fade-up sw-toast" style={{
    position: "fixed", bottom: 32, right: 32, zIndex: 5000,
    padding: "16px 24px", borderRadius: T.radiusXl,
    background: T.surfaceStrong, backdropFilter: "blur(24px)",
    border: `1px solid ${T.border}`, borderLeft: `6px solid ${c}`,
    color: T.text, fontSize: 14, fontWeight: 600, boxShadow: T.shadowXl,
    display: "flex", alignItems: "center", gap: 12, maxWidth: "calc(100vw - 32px)"
  }}>
    <div style={{ width: 10, height: 10, borderRadius: "50%", background: c, boxShadow: `0 0 10px ${c}`, flexShrink: 0 }} />
    <span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{msg}</span>
  </div>;
}

/* ── Modal — full screen scroll fix + mobile safe ────────────── */
  ...
  return (
    <div onClick={onClose} className="modal-overlay fade-up" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, width: "100vw", height: "100vh", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center", padding: "12px" }}>
      <div className="glass-strong spring-in" onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: width, borderRadius: T.radiusXl, maxHeight: "calc(100vh - 24px)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${T.borderSubtle}`, display: "flex", justifyContent: "space-between", alignItems: "center", background: T.surfaceStrong, flexShrink: 0 }}>
          <div style={{ fontWeight: 800, fontSize: 18, color: T.text, letterSpacing: "-0.02em", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", paddingRight: 8 }}>{title}</div>
          <button type="button" onClick={onClose} className="btn-ghost" style={{ padding: 6, borderRadius: "50%", flexShrink: 0 }}><X size={18} /></button>
        </div>
        <div style={{ padding: "16px 20px", overflowY: "auto", background: T.surfaceGlass, flex: 1, minHeight: 0, overflowX: "hidden", WebkitOverflowScrolling: "touch" }}>{children}</div>
        {footer && <div style={{ padding: "16px 20px", borderTop: `1px solid ${T.borderSubtle}`, background: T.surfaceStrong, display: "flex", justifyContent: "flex-end", gap: 12, flexShrink: 0, flexWrap: "wrap" }}>{footer}</div>}
      </div>
    </div>
  );
}
...
export function KCard({ label, value, sub, icon: Icon, color, delta, onClick, active }) {
  ...
  return <div className="glass" style={{ padding: "20px", borderRadius: T.radius, cursor: onClick ? "pointer" : "default", border: active ? `2px solid ${c}` : undefined, display: "flex", flexDirection: "column", height: "100%" }} onClick={onClick}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: T.textSub, marginBottom: 6, letterSpacing: "0.05em", textTransform: "uppercase" }}>{label}</div>
        <div style={{ fontSize: 24, fontWeight: 800, color: T.text, letterSpacing: "-0.03em", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{value}</div>
      </div>
      <div style={{ width: 40, height: 40, borderRadius: T.radius, background: `${c}15`, display: "flex", alignItems: "center", justifyContent: "center", color: c, flexShrink: 0 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: "auto" }}>
      ...
      {sub && <span style={{ fontSize: 11, color: T.textMuted, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sub}</span>}
    </div>
  </div>;
}
  ...
  return (
    <Modal open={true} onClose={onClose} title="Confirm Deletion" width={420}>
      ...
      <Field label="Enter password to confirm" req>
        <GIn type="password" value={pass} onChange={e=>{setReactPass(e.target.value);setErr("");}} onKeyDown={e=>e.key==="Enter"&&go()} />
      </Field>
...
export function Pager({ total, page, ps, setPage, setPs }) {
  ...
  const nb = dis => ({ padding: "8px 12px", minWidth: 34, borderRadius: T.radius, border: `1px solid ${T.border}`, cursor: dis ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 13, background: T.surfaceGlass, color: dis ? T.textMuted : T.textSub, opacity: dis ? .5 : 1, transition: "all 0.2s" });
  const range = [];
  for (let i = s; i <= e; i++) range.push(i);
  return <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", borderTop: `1px solid ${T.border}`, flexWrap: "wrap", background: T.surfaceGlass, borderRadius: `0 0 ${T.radius} ${T.radius}` }}>
    <span style={{ fontSize: 12, color: T.textMuted, flex: 1, minWidth: 100, fontWeight: 500 }}>{Math.min((page - 1) * ps + 1, total)}–{Math.min(page * ps, total)} of <span style={{ fontWeight: 800, color: T.text }}>{total}</span></span>
    <select className="sel" value={ps} onChange={e => { setPs(Number(e.target.value)); setPage(1); }} style={{ width: "auto", padding: "6px 28px 6px 12px", fontSize: 12, fontWeight: 700, borderRadius: T.radiusFull }}>
    ...
    <div style={{ display: "flex", gap: 4 }}>
      ...
      {range.map(p => <button key={p} onClick={() => setPage(p)} style={{ padding: "8px 12px", minWidth: 34, borderRadius: T.radius, border: p === page ? `1px solid ${T.accent}` : `1px solid ${T.border}`, cursor: "pointer", fontWeight: 700, fontSize: 13, background: p === page ? T.accent : T.surfaceGlass, color: p === page ? "#fff" : T.textSub, transition: "all 0.2s ease" }}>{p}</button>)}
