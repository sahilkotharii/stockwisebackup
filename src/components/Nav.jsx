...
const ROLE_ACCESS = {
  ...
  manager: ["dashboard","sales","new-sale-invoice","new-proforma","sales-orders","sales-return","sales-reports","purchase","new-purchase-bill","purchase-orders","purchase-return","purchase-reports","inventory","products","categories","product-reports","pnl","ledger","receipts","payments","vendors","transactions","settings-profile","settings-theme","how-to-use"],
  sales: ["sales","new-sale-invoice","new-proforma","sales-orders","sales-return","sales-reports","inventory","products","vendors","ledger","receipts","settings-profile","settings-theme","how-to-use"],
  purchase: ["purchase","new-purchase-bill","purchase-orders","purchase-return","purchase-reports","inventory","products","vendors","ledger","payments","settings-profile","settings-theme","how-to-use"],
  accountant: ["sales","purchase","sales-return","purchase-return","pnl","ledger","receipts","payments","expenses-incomes","vendors","transactions","settings-profile","settings-theme","how-to-use"],
...
export default function Sidebar({ page, setPage, user, onLogout, ctx }) {
  ...
  useEffect(() => {
    ...
    if (g) setOpen({ [g.id]: true });
  }, [page]);

  // Accordion mode: opening one group closes all others
  const toggle = id => setOpen(p => ({ [id]: !p[id] }));
    ...
    <div className="spring-in" style={{ position: "fixed", left: 0, top: 0, bottom: 0, width: 260, zIndex: 201, background: T.surfaceStrong, backdropFilter: "blur(24px)", display: "flex", flexDirection: "column", boxShadow: T.shadowXl }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 0" }}>
        {groups.map(g => {
          ...
          return <div key={g.id}>
            <button onClick={() => setOpenGrp(p => ({ [g.id]: !p[g.id] }))} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 16px", width: "100%", border: "none", background: "transparent", cursor: "pointer", fontSize: 11, fontWeight: 800, color: g.children.some(c => c.id === page) ? T.accent : T.textMuted, textTransform: "uppercase", letterSpacing: ".05em", textAlign: "left" }}>
...
export function TopBar({ page, user, syncSt, onSync, toggleTheme, isDark, setPage, ctx, onLogout, onDrawerOpen }) {
  const T = useT();
  const titles = {"dashboard":"Dashboard","sales":"Sales Bills","new-sale-invoice":"New Tax Invoice","new-proforma":"New Proforma Invoice","sales-orders":"Sales Orders","sales-return":"Sales Return","sales-reports":"Sales Reports","purchase":"Purchase Bills","new-purchase-bill":"New Purchase Bill","purchase-orders":"Purchase Orders","purchase-return":"Purchase Return","purchase-reports":"Purchase Reports","inventory":"Stock Status","products":"Products","categories":"Categories","product-reports":"Product Reports","pnl":"Profit & Loss","ledger":"Ledger","receipts":"Receipts","payments":"Payments","expenses-incomes":"Expenses & Incomes","vendors":"Vendors","transactions":"Transactions","approvals":"Approvals","settings-profile":"Profile","settings-theme":"Theme","how-to-use":"How to Use","admin-users":"Users","admin-activity":"Activity Log","admin-sessions":"Login History","admin-invoice":"Invoice Template","admin-series":"Bill Series","admin-sheets":"Google Sheets","admin-export":"Export Data"};
          ...
          {showNotif && <>
            ...
            <div className="glass-strong spring-down notif-dropdown" onClick={e => e.stopPropagation()} style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: 300, borderRadius: T.radius, overflow: "hidden", zIndex: 200 }}>
              ...
              <div style={{ maxHeight: 320, overflowY: "auto", padding: "8px 14px" }}>
                {oosCnt > 0 && <div onClick={() => { setShowNotif(false); ctx.setNavAction?.({ filter: "oos" }); setPage("inventory"); }} style={{ padding: "8px 10px", borderRadius: T.radius, background: T.redBg, marginBottom: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, color: T.red, display: "flex", alignItems: "center", gap: 6 }}><AlertOctagon size={13} />{oosCnt} Out of Stock</div>}
                {lowCnt > 0 && <div onClick={() => { setShowNotif(false); ctx.setNavAction?.({ filter: "low" }); setPage("inventory"); }} style={{ padding: "8px 10px", borderRadius: T.radius, background: T.amberBg, marginBottom: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, color: T.amber, display: "flex", alignItems: "center", gap: 6 }}><AlertTriangle size={13} />{lowCnt} Low Stock</div>}
                {user.role === "admin" && pending.length > 0 && <div onClick={() => { setShowNotif(false); setPage("approvals"); }} style={{ padding: "8px 10px", borderRadius: T.radius, background: T.accentBg, marginBottom: 6, cursor: "pointer", fontSize: 12, fontWeight: 700, color: T.accent, display: "flex", alignItems: "center", gap: 6 }}><CheckSquare size={13} />{pending.length} Pending Approvals</div>}
