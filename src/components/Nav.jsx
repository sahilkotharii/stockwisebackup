...
import { LayoutDashboard, TrendingUp, ShoppingCart, RotateCcw, Package, BarChart2, PieChart, Tag, Users, ArrowLeftRight, CheckSquare, Settings, LogOut, Bell, RefreshCw, Sun, Moon, Layers, X, User, Download, Hexagon, ChevronLeft, ChevronRight, Activity } from "lucide-react";
...
function ProfileDropdown({ user, onClose, setPage, setSettingsTab, onLogout }) {
  ...
  return (
    <div className="glass-strong spring-down" onClick={e => e.stopPropagation()}
      style={{ position: "absolute", right: 0, top: "calc(100% + 16px)", width: 240, borderRadius: T.radius, padding: 8, zIndex: 200 }}>
...
function NotificationDropdown({ ctx, user, setPage, onClose }) {
  const T = useT();
  const { changeReqs, actLog } = ctx;
  const pending = (changeReqs || []).filter(r => r.status === "pending");
  const recentLogs = (actLog || []).slice(0, 5);

  return (
    <div className="glass-strong spring-down" onClick={e => e.stopPropagation()}
      style={{ position: "absolute", right: 0, top: "calc(100% + 16px)", width: 320, borderRadius: T.radius, overflow: "hidden", zIndex: 200 }}>
      
      <div style={{ padding: "16px 20px", background: T.surfaceStrong, borderBottom: `1px solid ${T.borderSubtle}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontWeight: 800, fontSize: 14, color: T.text }}>Notifications</div>
        {user.role === "admin" && pending.length > 0 && (
          <span style={{ background: T.redBg, color: T.red, fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 99 }}>{pending.length} Pending</span>
        )}
      </div>
      ...
      <div style={{ maxHeight: 360, overflowY: "auto" }}>
        {user.role === "admin" && pending.length > 0 && (
          <div style={{ padding: "12px 20px", borderBottom: `1px solid ${T.borderSubtle}` }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.textMuted, marginBottom: 8, letterSpacing: "0.05em" }}>NEEDS APPROVAL</div>
            <button onClick={() => { setPage("approvals"); onClose(); }} className="glass" style={{ width: "100%", textAlign: "left", padding: 12, borderRadius: T.radius, border: `1px solid ${T.accent}30`, background: T.accentBg, cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
              <CheckSquare size={16} color={T.accent} />
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>You have {pending.length} pending requests</div>
                <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>Click to review and approve</div>
              </div>
            </button>
          </div>
        )}
        ...
        <div style={{ padding: "12px 20px" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: T.textMuted, marginBottom: 8, letterSpacing: "0.05em" }}>RECENT ACTIVITY</div>
          {recentLogs.length === 0 ? (
  ...
  return <>
    {showMenu && (
      <div style={{ position: "fixed", inset: 0, zIndex: 150, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }} onClick={() => setShowMenu(false)}>
        <div className="glass-strong spring-down" onClick={e => e.stopPropagation()} style={{ position: "fixed", bottom: 80, left: 16, right: 16, borderRadius: T.radiusXl, padding: "20px", maxHeight: "70vh", overflowY: "auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            {allItems.map(n => (
              <button key={n.id} onClick={() => { setPage(n.id); setShowMenu(false); }}
                style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "16px 8px", borderRadius: T.radius, border: `1px solid ${page === n.id ? T.accent : T.border}`, background: page === n.id ? T.accentBg : T.surface, cursor: "pointer", transition: "all 0.2s ease" }}>
    ...
    <div className="mobile-nav" style={{ boxShadow: "0 -4px 24px rgba(0,0,0,0.1)" }}>
      {mainItems.map(n => {
        const isActive = page === n.id;
        return (
          <button key={n.id} onClick={() => setPage(n.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, padding: "14px 4px", border: "none", background: "transparent", cursor: "pointer", color: isActive ? T.accent : T.textMuted, transition: "color 0.2s ease" }}>
      ...
      {hasMore && (
        <button onClick={() => setShowMenu(true)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, padding: "14px 4px", border: "none", background: "transparent", cursor: "pointer", color: showMenu ? T.accent : T.textMuted, transition: "color 0.2s ease" }}>
          <Layers size={24} />
    ...
    <div style={{ position: "sticky", top: 0, zIndex: 40, background: T.surfaceGlass, backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", padding: "16px 20px", margin: "-12px -16px 24px -16px", borderBottom: `1px solid ${T.borderSubtle}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={onSync} className="btn-ghost hide-mob" style={{ padding: "8px 16px", fontSize: 13, borderRadius: 30, background: T.surface, boxShadow: T.shadow }}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => { setShowNotif(!showNotif); setShowProfile(false); }} className="btn-ghost" style={{ width: 40, height: 40, padding: 0, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: T.surface, boxShadow: T.shadow }}>
              <Bell size={18} />
              {badgeCnt > 0 && <span style={{ position: "absolute", top: 8, right: 10, width: 8, height: 8, borderRadius: "50%", background: T.red, border: `2px solid ${T.surface}`, boxShadow: `0 0 8px ${T.red}` }} />}
            </button>
            {showNotif && (
              <>
