...
export default function Inventory({ ctx }) {
  ...
  // Respond to notification click → auto-filter
  useEffect(() => {
    const a = ctx.consumeNavAction?.();
    if (a?.filter && ["low","oos","healthy","all"].includes(a.filter)) setStockF(a.filter);
  }, []);
    ...
    <div className="glass fade-up" style={{ borderRadius: T.radius, overflow: "hidden" }}>
      ...
      <div className="mob-card-view">
        {filtered.slice((pg-1)*ps, pg*ps).map(p => {
          ...
          const stockColor = p.stock <= 0 ? T.red : p.stock <= Number(p.minStock || 0) ? T.amber : T.green;
          return (
            <div key={p.id} style={{ borderBottom: `1px solid ${T.borderSubtle}`, padding: "14px 16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
                  {p.imageUrl && <img src={p.imageUrl} alt="" style={{ width: 36, height: 36, borderRadius: T.radius, objectFit: "cover", flexShrink: 0 }} onError={e => e.target.style.display = "none"} />}
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 700, color: T.text, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</div>
                    <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2, fontWeight: 500 }}>{p.sku}{cat ? ` · ${cat.name}` : ""}</div>
                  </div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div style={{ fontSize: 18, fontWeight: 800, color: stockColor }}>{p.stock}</div>
                  <StChip stock={p.stock} min={Number(p.minStock || 0)} />
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 10, fontSize: 11 }}>
                <div><span style={{ color: T.textMuted }}>Open: </span><span style={{ fontWeight: 700 }}>{p.opening}</span></div>
                <div><span style={{ color: T.blue }}>+Buy: </span><span style={{ fontWeight: 700, color: T.blue }}>{p.purchased}</span></div>
                <div><span style={{ color: T.red }}>-Sold: </span><span style={{ fontWeight: 700, color: T.red }}>{p.sold}</span></div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: T.accent }}>{fmtCur(p.value)}</span>
                {isAdmin && <button className="btn-ghost liquid-trans" onClick={() => openEditOs(p)} style={{ padding: "4px 10px", fontSize: 11 }}><Edit2 size={12} style={{ marginRight: 4 }} />Edit Stock</button>}
              </div>
            </div>
          );
        })}
        {filtered.length === 0 && <div style={{ padding: "40px 0", textAlign: "center", color: T.textMuted, fontSize: 13, fontWeight: 600 }}>No products found</div>}
        <div style={{ padding: "12px 16px", borderTop: `2px solid ${T.accent}30`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontWeight: 800, fontSize: 13, color: T.text }}>Total Value</span>
          <span style={{ fontWeight: 800, fontSize: 16, color: T.accent }}>{fmtCur(totalValue)}</span>
        </div>
      </div>
