...
import { Plus, X, Trash2, Eye, Edit2, FileText, Package } from "lucide-react";
...
const STATUS_COLORS = { "Draft": "amber", "Sent": "blue", "Confirmed": "accent", "Fulfilled": "green", "Cancelled": "red" };

export default function SalesOrders({ ctx }) {
  const T = useT();
  const { salesOrders = [], saveSalesOrders, products, vendors, user, addLog, invoiceSettings } = ctx;
  ...
  const blank = { date: today(), vendorId: "", status: "Draft", validUntil: "", notes: "", items: [{ id: uid(), productId: "", qty: 1, price: "" }] };
  const [form, setForm] = useState(blank);
  const f = (k, v) => setForm(p => ({ ...p, [k]: v }));
  const upItem = (id, k, v) => setForm(p => ({ ...p, items: p.items.map(i => {
    if (i.id !== id) return i;
    const u = { ...i, [k]: v };
    if (k === "productId") { const pr = products.find(x => x.id === v); if (pr) u.price = pr.mrp || ""; }
    return u;
  }) }));
  ...
  const save = () => {
    ...
    if (edit) {
      saveSalesOrders(salesOrders.map(o => o.id === edit.id ? { ...edit, ...form, items: valid, total, updatedTs: new Date().toISOString() } : o));
      addLog("edited", "sales order", edit.orderNo);
    } else {
      const prefix = invoiceSettings?.soSeries || "SO-";
      const startNum = Number(invoiceSettings?.soSeriesStart || 1);
      const cnt = salesOrders.length;
      ...
      addLog("created", "sales order", orderNo, `${valid.length} items · ${fmtCur(total)}`);
    }
    setModal(false);
  };
  ...
  ).sort((a, b) => (b.date || "").localeCompare(a.date || "")), [salesOrders, df, dt, statusF, search]);

  const stats = {
    total: filtered.length,
    pending: filtered.filter(o => ["Draft", "Sent", "Confirmed"].includes(o.status)).length,
    fulfilled: filtered.filter(o => o.status === "Fulfilled").length,
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
      ...
      <GBtn onClick={openNew} icon={<Plus size={14} />}>New Sales Order</GBtn>
    </div>

    <div className="kgrid">
      <KCard label="Total Orders" value={String(stats.total)} sub="In selected period" icon={FileText} color={T.accent} />
      <KCard label="Pending" value={String(stats.pending)} sub="Draft / Sent / Confirmed" icon={Package} color={T.amber} />
      <KCard label="Fulfilled" value={String(stats.fulfilled)} sub="Completed orders" icon={Package} color={T.green} />
    ...
    <div className="glass" style={{ borderRadius: T.radius, overflow: "hidden" }}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{["Date","Order No","Customer","Items","Total","Status","Actions"].map((h,i) => <th key={i} className="th" style={{ textAlign: ["Items","Total"].includes(h) ? "right" : "left" }}>{h}</th>)}</tr></thead>
          <tbody>{filtered.slice((pg-1)*ps, pg*ps).map(o => {
            ...
            return <tr key={o.id} className="trow">
              ...
              <td className="td" style={{ fontWeight: 700, color: T.accent, whiteSpace: "nowrap" }}>{o.orderNo}</td>
              ...
              <td className="td">
                <div style={{ display: "flex", gap: 4, justifyContent: "flex-end", alignItems: "center", flexWrap: "wrap" }}>
                  <ConversionMenu source={o} sourceType="sales-order" ctx={ctx} />
        ...
        {filtered.length === 0 && <div style={{ padding: 40, textAlign: "center", color: T.textMuted }}>No sales orders found</div>}
      </div>
      <Pager total={filtered.length} page={pg} ps={ps} setPage={setPg} setPs={setPs} />
    </div>

    <Modal open={modal} onClose={() => setModal(false)} title={edit ? `Edit: ${edit.orderNo}` : "New Sales Order"} width={760}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div className="fgrid">
          ...
          <Field label="Valid Until"><GIn type="date" value={form.validUntil || ""} onChange={e => f("validUntil", e.target.value)} /></Field>
        </div>
        ...
        {form.items.map((item, i) => (
          <div key={item.id} style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, borderRadius: T.radius, border: `1px solid ${T.borderSubtle}` }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 8, alignItems: "end" }}>
              ...
              <Field label="Price"><GIn type="number" min="0" step="0.01" value={item.price} onChange={e => upItem(item.id, "price", e.target.value)} /></Field>
        ...
        <div style={{ textAlign: "right", fontSize: 16, fontWeight: 800, color: T.accent }}>Total: {fmtCur(total)}</div>
      </div>
    </Modal>

    <DeleteConfirmModal open={!!delConfirm} onClose={() => setDelConfirm(null)} 
      onConfirm={() => { saveSalesOrders(salesOrders.filter(o => o.id !== delConfirm.id)); addLog("deleted", "sales order", delConfirm.orderNo); }} 
