...
.glass-strong {
  ...
  border: 1px solid ${T.border};
  border-top: 1px solid ${T.glassShine};
  box-shadow: inset 0 1px 1px ${T.glassShine}, ${T.shadowXl};
}
/* Cards/containers that should NOT lift on hover */
.glass.no-lift:hover, .glass-form:hover { transform: none; }
.glass-form {
  background: ${T.surfaceGlass};
  backdrop-filter: blur(20px);
  -webkit-backdrop-filter: blur(20px);
...
@media(max-width:768px){
  ...
  .mobile-nav{display:flex;position:fixed;bottom:0;left:0;right:0;z-index:100;background:${T.surfaceStrong};backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border-top:1px solid ${T.border};justify-content:space-evenly;align-items:stretch;padding-bottom:env(safe-area-inset-bottom,0px)}
  .desktop-sidebar{display:none!important}

  .main-wrap{margin-left:0!important;padding:12px 12px 96px!important;overflow-x:hidden!important;width:100%}

  .kgrid{grid-template-columns:1fr 1fr!important;gap:12px!important}
  .fgrid{grid-template-columns:1fr!important;gap:12px!important}
  .fgrid .s2{grid-column:1!important}
  .chart-row { grid-template-columns:1fr!important; }

  .filter-wrap{gap:8px!important;flex-wrap:wrap!important;width:100%}

  .period-bar-wrap { width: 100%; flex-direction: row; flex-wrap: wrap; gap: 8px !important; padding: 8px !important; }
  ...
  .mob-bill-card:active { background: ${T.isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.02)"}; }
}
